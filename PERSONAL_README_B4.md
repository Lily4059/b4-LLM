...
> 说明：本模板适用于 B 方向 Agent 智能体实训。B4 模块负责本地 LLM 推理与工具决策，同时个人还负责整体前端实现（Agent Studio Web 与 Module Studio b4模块演示系统）。
...
| 模型 | 来源 | 项目内相对路径 | 用途 |
|---|---|---|---|
| Qwen3.5-4B | [ModelScope](https://www.modelscope.cn/models/Qwen/Qwen3.5-4B) | `configs/model.yaml` 中配置为 `/root/siton-tmp/assignment_B/Qwen3.5-4B` | 主模型，用于 prompt_json 和 native_tools 推理 |
...
| `messages_module_demo_plan_execute.json` | 基于模块演示的 Plan-and-Execute 样例 |
...
### 4.6 基础功能结果截图
![接收初始 messages，生成包含 tool_calls 的 AIMessage](图片/基础功能结果截图/接收初始%20messages，生成包含%20tool_calls%20的%20AIMessage.png)

![接收已包含 ToolMessage 的 messages，生成最终回答](图片/基础功能结果截图/接收已包含%20ToolMessage%20的%20messages，生成最终回答.png)

![工具调用失败后的模型回答](图片/基础功能结果截图/工具调用失败后的模型回答.png)

![mock 模式（无 GPU 环境）](图片/基础功能结果截图/mock%20模式（无%20GPU%20环境）.png)

![关键输出文件截图](图片/基础功能结果截图/关键输出文件截图.png)
...
# 多工具往返演示（prompt_json 模式，真实模型验证）
python b4_local_agent_llm.py --model_config ../configs/model.yaml \
  --messages ../data/messages/messages_no_tool_multi_call_test.json \
  --tools_schema ../data/messages/tools_schema_basic.json \
  --mode prompt_json --outdir ../outputs/B4_llm/multi_tool_real
...
#### 示例图片
![多工具并行调用命令](图片/多工具并行调用/多工具并行调用命令.png)

![多工具并行网页端](图片/多工具并行调用/多工具并行网页端.png)

![多工具并行关键输出文件截图](图片/多工具并行调用/关键输出文件截图.png)
...
python b4_local_agent_llm.py --model_config ../configs/model.yaml \
  --messages ../data/messages/messages_module_demo_plan_execute.json \
  --tools_schema ../data/messages/tools_schema_basic.json \
...
#### 示例图片
![Plan-and-Execute 命令](图片/Plan-and-Execute%20多步规划/Plan-and-Execute命令.png)

![Plan-and-Execute 网页端](图片/Plan-and-Execute%20多步规划/Plan-and-Execute网页端.png)

![Plan-and-Execute 计划执行](图片/Plan-and-Execute%20多步规划/Plan-and-Execute%20计划执行.png)

![progress.md 进度截图](图片/Plan-and-Execute%20多步规划/progress.md%20进度截图.png)
...
#### 示例图片
![模型切换运行网页版4B](图片/本地模型切换/模型切换运行网页版4B.png)

![模型切换运行7B](图片/本地模型切换/模型切换运行7B.png)
...
#### 示例图片
![双模式对比运行](图片/双模式对比运行/双模式对比运行.png)

![双模式对比关键输出文件截图](图片/双模式对比运行/关键输出文件截图.png)
...
#### 示例图片
![批量评估运行过程截图](图片/批量评估/批量评估运行过程截图.png)

![批量评估 prompt_json](图片/批量评估/批量评估prompt.png)

![批量评估 native_tools](图片/批量评估/评论评估native.png)

![批量评估 adaptive](图片/批量评估/批量评估adaptive.png)
